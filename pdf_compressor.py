...
def compress_file(fileName, compression_level=1):
    fileName_out = fileName[:-4]+'-compressed.pdf'
    if not os.path.exists(fileName_out):
        compress(fileName, fileName_out, power=compression_level)
    else:
        print("Compressed version already exists:", fileName_out)
...
def compress_folder():
    ...
    for pdf_file in pdf_files_list:
        ...
        if 'compressed.pdf' in pdf_file:
            ...
            compress_file(pdf_file, compression_level)
        count+=1
